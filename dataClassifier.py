...
def enhancedPacmanFeatures(state, action):
    ...
    for g in newGhostStates:
        cGost = min(cGost, util.manhattanDistance(newPos, g.getPosition()))

    # score
    score = state.getScore()
        
    featureCounter['fNumb'] = 1.0/(fNumb+1.0)
    featureCounter['cGost'] = 1.0/(cGost+1.0)
    featureCounter['cFood'] = 1.0/(cFood+1.0)
    featureCounter['score'] = score
...
def runClassifier(args, options):
    ...
    if((options.odds) & (options.classifier == "naiveBayes" or (options.classifier == "nb")  or (options.classifier == "mira")) ):
